...
class Sale:
    ...
    def _get_grouped_invoice_date(self, invoice_type):
        date = None
        if self.invoice_method == 'shipment':
            for line in self.lines:
                for iline in line.get_invoice_line(invoice_type):
    ...
    def _get_grouped_invoice_domain(self, invoice):
        ...
        if self.invoice_grouping_method == 'standard' and period:
            date = self._get_grouped_invoice_date(invoice.type)
        ...
        if self.invoice_grouping_method == 'standard' and period:
            for code in [Transaction().language, 'en_US']:
                ...
                if langs:
                    break
            lang, = langs
            date = self._get_grouped_invoice_date(invoice_type)
            start, end = self._get_invoice_dates(date,
                self.party.sale_invoice_grouping_period)
